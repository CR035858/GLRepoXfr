...
//		.withUser("admin").password(encoder().encode("admin123")).roles("ADMIN");
	...
	@Override
		...
		http.authorizeRequests()
		.antMatchers(HttpMethod.GET,"/", "/library/books/list","/library/books/showFormForAdd","library/books/403")
		.hasAnyAuthority("USER", "ADMIN")
		.antMatchers(HttpMethod.POST,"library/books/save")
		.hasAnyAuthority("USER", "ADMIN")
		.antMatchers("/library/books/showFormForUpdate","/library/books/delete")
		.hasAnyAuthority("ADMIN")
		.anyRequest().authenticated()
		.and()
		.formLogin().loginProcessingUrl("/login")//.successForwardUrl("/library/books/list")
		.permitAll()
		.and()
		.logout().logoutSuccessUrl("/login").permitAll()
		.and()
		.exceptionHandling().accessDeniedPage("/library/books/403")
		.and()
	...
	/*	http.authorizeRequests()
		 ...
		 * http.authorizeRequests()
	         ...
	         //  .antMatchers("/books/showFormForUpdate","/books/delete").hasAnyAuthority("ADMIN")
	           .antMatchers("/books/showFormForUpdate","/books/delete").hasAuthority("ADMIN")
	            //  .antMatchers("/books/showFormForUpdate","/books/delete").hasAuthority("USER")
	            .anyRequest().authenticated()
	            .and()
	            .formLogin().loginProcessingUrl("/login").successForwardUrl("/books/list").permitAll()
	            .and()
